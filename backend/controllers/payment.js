...
const approvePaypalOrder = async (req, res) => {
    ...
    res.send({ message: "Ti abbiamo mandato una e-mail di verifica", mail: result });
}
const capturePaypalOrder = async (req, res) => {
    try {
        ...
        return res.redirect(clientUrl + "/verification-successful/" + itemId);
    } catch (err) {
        
        return res.redirect(clientUrl + "/verification-error");
    }
}
...
const createPaymentIntent = async (req, res) => {
    ...
    if(req.session.userEmail) user = await User.findOne({ email: req.session.userEmail });
   
    if(!user) {
        password = await register(name, email, password)
    }else {
        name = user.name;
        email = user.email;
        password = user.password;
    ...
    return res.send({ message: "Ti abbiamo inviato una email di verifica" });
}
