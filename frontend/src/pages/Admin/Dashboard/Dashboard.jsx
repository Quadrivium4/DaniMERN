                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="actions">
                                        ...
                                        <button
                                            onClick={() => {
                                                navigate("edit-subcourse", { state: { subcourse, action: "update" } });
                                                // setPopContent(
                                                //     <EditSubcourse
                                                //         subcourse={subcourse}
                                                //         action="update"
...
export const EditSubcourse = ({ subcourse, action}) =>{
    const imgPreview = useRef(null)
    ...
    const [subcourseVideos, setSubcourseVideos] = useState(
        {
           main: []        
        }
    );
    useEffect(() =>{
        getSubcourse(subcourse._id).then(({data})=>{
            console.log(data);
            let newSubcourseVideos = { main: [] };
            for (let i = 0; i < data.medias.length; i++) {
                const video = data.medias[i];
                if(!video.section){
                    newSubcourseVideos.main.push(video);
                }else if(video.section && newSubcourseVideos.hasOwnProperty(video.section)){
                    newSubcourseVideos[video.section].push(video);
                }else if(video.section && !newSubcourseVideos.hasOwnProperty(video.section)){
                    newSubcourseVideos[video.section] = [video];
                }
    ...
    return (
        <>
            ...
            <input
                ...
                onChange={(e) =>
                    setSubcourseData({ ...subcourseData, name: e.target.value })
                }
            />
            <p>Course Description:</p>
            <input
            ...
            <div>
                ...
                {Object.entries(subcourseVideos).map(([section, videos]) => {
